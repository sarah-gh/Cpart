    ...
    <header class="header">
        <div class="header-container">
            ...
            <section class="flexContainer row" v-show="!$store.state.login">
                <div class="flex gap-3">
                    ...
                    <div class="search-container">
                        <img src="../../../../assets/img/search-normal.svg" class='search nav-top icon'
                            @click="clickIsVisibleSearch()" alt="search">
                        <transition name="fade">
                            <header-search v-if="isVisibleSearch" v-model='thirdExample' :items='ppp' :shows='"name"'
                                :add-btn='true' @selected='thirdExampleSelected'></header-search>
                        </transition>
                    </div>
                </div>
                <div class="logo">
            ...
            <div class="buttons" v-show="$store.state.login">
                <div>
                    <div class="relative inline-block" @mouseover="isVisible = true" @mouseleave="isVisible = false"
                        @keydown.enter="isVisible = !isVisible">
                        <!-- <router-link :to="'panel/profile/0'" class="profile_user"> -->
                        <div class="avatar_img">
                            <div @click="profileuser()" type="button"
                                class=" inline-flex items-center justify-between py-1 font-medium text-gray-700 transition-all duration-500 rounded-md focus:outline-none focus:text-brand-900 sm:focus:shadow-outline">
                                <img v-if="userLogin.userphoto" :src="userLogin.userphoto"
                                    class='flex-shrink-0 avatar nav-top' alt="user-photo">
                                <img v-else src="../../../../assets/img/default-profile.jpg"
                                    class='flex-shrink-0 avatar nav-top' alt="user-photo">
                            </div>
                        </div>
                        <!-- </router-link> -->
